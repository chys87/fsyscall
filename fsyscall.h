...
#define fsys_generic_base(sysno,rettype,clobbermem,argc,...)		\
	( {																\
		...
		register unsigned long r__ = (sysno);						\
		FSYS_GENERIC_LOAD_REGS_##argc;								\
		__asm__ __volatile__ ("syscall" :							\
				"=a"(r__) :											\
				"a"(r__) FSYS_GENERIC_ASM_REGS_##argc :				\
				FSYS_MEMORY_##clobbermem "cc", "r11", "cx");		\
		r__;														\
	} )
...
#define FSYS_LOAD_REGS_1(ta)				register ta A __asm__("rdi") = a
#define FSYS_LOAD_REGS_2(ta,tb)				FSYS_LOAD_REGS_1(ta); register tb B __asm__("rsi") = b
#define FSYS_LOAD_REGS_3(ta,tb,tc)			FSYS_LOAD_REGS_2(ta,tb); register tc C __asm__("rdx") = c
...
#define FSYS_GENERIC_LOAD_REGS_1	register __typeof__((a__)) A__ __asm__("rdi") = a__
#define FSYS_GENERIC_LOAD_REGS_2	FSYS_GENERIC_LOAD_REGS_1; register __typeof__((b__)) B__ __asm__("rsi") = b__
#define FSYS_GENERIC_LOAD_REGS_3	FSYS_GENERIC_LOAD_REGS_2; register __typeof__((c__)) C__ __asm__("rdx") = c__
...
def_fsys(pwritev_raw,pwritev,long,5,int,const struct iovec *,unsigned long,unsigned long,unsigned long)
#define fsys_pwritev(a,b,c,d) fsys_pwritev_raw(a,b,c,d,0)
def_fsys(pwrite,pwrite64,long,4,int,const void *,unsigned long,unsigned long)
def_fsys(pread,pread64,long,4,int,void *,unsigned long,unsigned long)
